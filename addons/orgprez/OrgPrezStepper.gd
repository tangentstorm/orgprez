...
func set_org(o:OrgNode):
	...
	this_audio_chunk = tracks[Org.Track.AUDIO].this_chunk()
	next_audio_chunk = tracks[Org.Track.AUDIO].next_chunk()
	show_debug_state()

func _on_audio_finished():
	track_ready[Org.Track.AUDIO] = true
	this_audio_chunk = next_audio_chunk
	next_audio_chunk = tracks[Org.Track.AUDIO].find_next(Org.Track.AUDIO)
...
func track_to_step():
	...
	for track in Org.Track.values():
		if track_ready[track]:
			var chunk = this_audio_chunk if track == Org.Track.AUDIO else tracks[track].this_chunk()
			if not chunk: continue
			if chunk.index < hindmost_index:
				hindmost_track = track
				hindmost_index = chunk.index
	# if hindmost_track != null: print("hindmost track:", Org.Track.keys()[hindmost_track])
...
func _process(_dt):
	...
	if playing or step_ready:
		step_ready = playing
		match track_to_step():
			Org.Track.EVENT: process_event_track()
			Org.Track.MACRO: process_macro_track()
			Org.Track.AUDIO: process_audio_track()
...
func show_debug_state():
	...
	ChunkList.highlight_chunk(tracks[Org.Track.EVENT].this_chunk(), Color.ROYAL_BLUE)
	ChunkList.highlight_chunk(tracks[Org.Track.MACRO].this_chunk(), Color.DARK_SLATE_GRAY)
	ChunkList.highlight_chunk(this_audio_chunk, Color.GOLDENROD if track_ready[Org.Track.AUDIO] else Color.DARK_GOLDENROD)
	ChunkList.highlight_chunk(next_audio_chunk, Color.SIENNA)

func process_event_track():
	if track_ready[Org.Track.EVENT] and track_ready[Org.Track.MACRO]:
		var event_chunk = tracks[Org.Track.EVENT].this_chunk()
		if not event_chunk: return
		if this_audio_chunk == null or event_chunk.index < this_audio_chunk.index:
			var script:String = tracks[Org.Track.EVENT].this_chunk().lines_to_string()
			if script == '@sync':
				...
				track_ready[Org.Track.EVENT] = false

func process_audio_track():
	if track_ready[Org.Track.AUDIO] and track_ready[Org.Track.MACRO] and track_ready[Org.Track.EVENT]:
		if this_audio_chunk:
			if this_audio_chunk.file_exists(org.get_dir()):
				track_ready[Org.Track.AUDIO] = false
...
func process_macro_track():
	if track_ready[Org.Track.MACRO] and track_ready[Org.Track.EVENT]:
		# fire if that chunk comes before the *next* audio track.
		var macro_chunk = tracks[Org.Track.MACRO].this_chunk()
